...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='? PrimeKG random split ??? 40 epoch probe?')
    parser.add_argument(
        '--processed-path',
        type=Path,
        default=Path('data/PrimeKG/processed/primekg_indication_mvp.pt'),
        help='?? random split pair ??? processed .pt ???',
    )
    parser.add_argument(
        '--nodes-csv',
        type=Path,
    ...
    parser.add_argument(
        '--output-json',
        type=Path,
        default=Path('outputs/random_split_probe_40epoch.json'),
        help='?????????',
    )
    return parser.parse_args()


def load_random_split_pairs(processed_path: Path) -> Dict[str, set[IdPair]]:
    obj = torch.load(processed_path, map_location='cpu', weights_only=False)
    if obj.get('split_mode') != 'random':
        raise ValueError(
            f'?? processed ???? random split?{processed_path}?split_mode={obj.get("split_mode")}'
        )
...
def derive_ho_triplets_for_pair_splits(
    ...
    """
    ??? PrimeKG ??????? drug-disease pair ??? drug-gene-disease ?????
    """
...
def run_probe(args: argparse.Namespace) -> None:
    ...
    print('\n[Prepare] Loading random split target pairs...')
    pair_splits = load_random_split_pairs(args.processed_path)
    for split_name, pair_set in pair_splits.items():
    ...
    if triplet_summary['train']['num_triplets'] == 0:
        raise RuntimeError('train split ????? HO triplets??????')
    if triplet_summary['valid']['num_triplets'] == 0 or triplet_summary['test']['num_triplets'] == 0:
        raise RuntimeError('valid/test split ????? HO triplets??????')
