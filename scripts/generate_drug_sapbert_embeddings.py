...
DEFAULT_INPUT_JSON = Path('data/drug_mechanism_texts.json')
DEFAULT_OUTPUT_PKL = Path('thick_drug_text_embeddings_sapbert.pkl')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Generate SapBERT drug embeddings from drug mechanism texts.',
    )
...
def load_mechanism_json(input_json: Path) -> Dict[str, Dict[str, str]]:
    ...
    for drug_id, value in payload.items():
        if not isinstance(value, dict):
            raise TypeError(f'Expected dict value for drug_id={drug_id}, got {type(value)}')
        normalized[str(drug_id)] = {
            'name': safe_text(value.get('name')),
            'mechanism_text': safe_text(value.get('mechanism_text')),
        }

    if not normalized:
        raise ValueError(f'No drug entries found in {input_json}')
    return normalized
...
def build_text_to_encode(name: str, mechanism_text: str) -> str:
    ...
    fallback_name = name if name else 'This drug'
    return f'{fallback_name} is a pharmacological agent.'
...
def build_embedding_dict(
    ...
    progress_bar = tqdm(
        ...
        desc='Encoding drug texts with SapBERT',
        unit='batch',
    )
    ...
    for batch in progress_bar:
        ...
        try:
            ...
            for drug_id, embedding in zip(batch_ids, batch_embeddings):
                embedding_dict[drug_id] = embedding.clone()
                success_count += 1
            ...
            for drug_id, name, mechanism_text in zip(batch_ids, batch_names, batch_mechanisms):
                try:
                    ...
                    embedding_dict[drug_id] = single_embedding.clone()
                    success_count += 1
                except Exception as single_exception:
                    print(
                        f'Warning: SapBERT single-item encoding failed for {drug_id!r}; '
                        f'fallback to zero. Error: {single_exception}'
                    )
                    embedding_dict[drug_id] = zero_embedding.clone()
...
def main() -> None:
    ...
    records = [
        (drug_id, item['name'], item['mechanism_text'])
        for drug_id, item in mechanism_payload.items()
    ]
    print(f'Total drugs to encode: {len(records)}')
    ...
    print(f"Total drugs: {stats['total_records']}")
